...
exports.initGoogle = function() {
    passport.use(new GoogleStrategy({
        ...
        callbackURL: `${config.url}/api/auth/google/callback`,
            ...
            const authId = `${config.auth.google.authIdPrefix}${profile.id}`
...
exports.initFacebook = function() {
    passport.use(new FacebookStrategy({
        ...
        enableProof: true,
            ...
            console.log('facebook login success, profile:', profile)
            const authId = `${config.auth.facebook.authIdPrefix}${profile.id}`
            userService.findByAuthId(authId)
                .then((user) => {
                    if (user) return done(null, user)
                    userService.create({
